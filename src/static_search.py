...
def prepare_static_search(out_folder):
  ...
  query_word_embeddings = np.array([
      embeddings_dict[w]
  ...
  query_question_distances = pairwise_distances(query_word_embeddings, question_word_embeddings)
  print(
    'query_question_distances.shape before filtering for good words',
    query_question_distances.shape,
  )
  good_word_mask = query_question_distances.min(axis=1) <= np.sqrt(good_word_threshold)
  ...
  for chunk_name, word_indices in word_indices_by_chunk.items():
    ...
    with (out_folder / f"chunk_{chunk_name}_words.json").open('w', encoding='utf-8') as f:
